...
def main(args):
    ...
    # ------------------------ Load Data ------------------------
    x = load_data(args.x_path)

    # ------------------------ kfold ------------------------
    kfold_loss = []
    kfold = KFold(n_splits=args.kfold, shuffle=True, random_state=args.seed)
    for i, (train_set, val_set) in enumerate(kfold.split(x, x)):
        # ------------------------ Instantiate Dataset ------------------------
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_save_path', default='/home/v-xiajiao/code/Med-TS-ExpertCL/checkpoints/debug_alb', type=str)
    parser.add_argument('--log_dir', default='/home/v-xiajiao/code/Med-TS-ExpertCL/logs/debug_alb', type=str)
